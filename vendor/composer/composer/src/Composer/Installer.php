...
class Installer
{
     ...
     * @return array
     */
    private function processDevPackages($localRepo, $pool, $policy, $repositories, $installedRepo, $lockedRepository, $transaction, array $operations = null)
    {
        if ($transaction === 'force-updates' && null === $operations) {
            throw new \InvalidArgumentException('Missing operations argument');
        }
        if ($transaction === 'force-links') {
            $operations = array();
        }
        ...
        foreach ($localRepo->getCanonicalPackages() as $package) {
            ...
            if ($this->update) {
                ...
                if ($this->updateWhitelist && !$this->isUpdateable($package)) {
                    ...
                    foreach ($currentPackages as $curPackage) {
                        if ($curPackage->isDev() && $curPackage->getName() === $package->getName() && $curPackage->getVersion() === $package->getVersion()) {
                            ...
                            } elseif ($transaction === 'force-updates') {
                ...
                if ($matches && $matches = $policy->selectPreferredPackages($pool, array(), $matches)) {
                    ...
                    if ($transaction === 'force-links' && $newPackage) {
