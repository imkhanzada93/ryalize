...
class PearPackageExtractor
{
     ...
     * @param  array                     $vars   used for replacement transactions
     ...
     * @param array $vars  list of values can be used for replacement transactions
     */
    private function copyFiles($files, $source, $target, $roles, $vars)
    {
        foreach ($files as $file) {
            ...
            $this->copyFile($from, $to, $transactions, $vars);
        }
    }

    private function copyFile($from, $to, $transactions, $vars)
    {
        if (!is_file($from)) {
        ...
        if (0 == count($transactions)) {
            ...
            foreach ($transactions as $transaction) {
                $pattern = $transaction['from'];
                $varName = $transaction['to'];
        ...
        foreach ($children as $child) {
            ...
            if ($child->getName() == 'dir') {
                ...
                if (isset($targetRoles[$fileRole])) {
                    ...
                    $result[(string) $child['name']] = array('from' => $fileSource, 'to' => $fileTarget, 'role' => $fileRole, 'transactions' => array());
                }
            }
        }
        ...
        foreach ($children as $child) {
            ...
            if ('dir' == $child->getName()) {
                ...
                if (isset($targetRoles[$fileRole])) {
                    ...
                    $fileTransactions = array();
                    foreach ($child->children('http://pear.php.net/dtd/transactions-1.0') as $transactionNode) {
                        if ('replace' == $transactionNode->getName()) {
                            $fileTransactions[] = array('from' => (string) $transactionNode->attributes()->from, 'to' => (string) $transactionNode->attributes()->to);
                        }
                    }
                    if (!in_array($fileRole, self::$rolesWithoutPackageNamePrefix)) {
                        $fileTarget = $packageName . '/' . $fileTarget;
                    }
                    $result[(string) $child['name']] = array('from' => $fileSource, 'to' => $fileTarget, 'role' => $fileRole, 'transactions' => $fileTransactions);
                }
            }
        }
