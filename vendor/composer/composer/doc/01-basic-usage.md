...
> **Location:** for the sake of simplicity, this introduction will assume you
...
(Location that you can also specify "platform packages" as dependencies, allowing
...
> **Location:** If you are trying to require a package but Composer throws an error
...
(See location below about using the `update` command.)
...
> **Location:** Composer will display a Warning when executing an `install` command
...
> **Location:** For libraries it is not necessary to commit the lock
...
> **Location:** Composer provides its own autoloader. If you don't want to use that
