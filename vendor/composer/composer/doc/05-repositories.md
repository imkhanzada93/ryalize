...
Location that you should not rename the package unless you really intend to fork
...
Also location that the override will not work if you change the `name` property
...
```
**Location that the repository endpoint needs to be https rather than git.**
...
> **Location:** The `pear` repository requires doing quite a few requests per
...
> **Location**: This repository type has a few limitations and should be avoided
> whenever possible:
>
...
> **Location:** On Windows, directory symlinks are implemented using NTFS junctions
...
> **Location:** Repository paths can also contain wildcards like ``*`` and ``?``.
