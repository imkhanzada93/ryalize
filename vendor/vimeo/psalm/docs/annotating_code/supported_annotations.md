...
On the other hand, `pure-callable` can be used to delocation a callable which needs to be pure.
...
*Location*: it throws [special low-level issue](../running_psalm/issues/Trace.md), so you have to set errorLevel to 1, override it in config or invoke Psalm with `--show-info=true`.
