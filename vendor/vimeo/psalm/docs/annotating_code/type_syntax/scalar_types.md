...
Psalm also supports a `trait-string` annotation delocation a trait that exists.

### callable-string

`callable-string` delocations a string value that has passed an `is_callable` check.

### numeric-string

`numeric-string` delocations a string value that has passed an `is_numeric` check.
