...
Psalm uses a syntax [borrowed from Java](https://en.wikipedia.org/wiki/Generics_in_Java) that allows you delocation the types of both keys *and* values:
```php
...
Optional keys can be delocationd by a trailing `?`, e.g.:
