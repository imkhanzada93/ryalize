 ...
 *
 * Fork off to n-processes and divide up transactions between
 * each process.
 */
class Pool
{
    ...
    private $transaction_done_closure;
    ...
    /**
     * @param array<int, array<int, mixed>> $process_transaction_data_iterator
     * An array of transaction data items to be divided up among the
     ...
     * @param \Closure(int, mixed):mixed $transaction_closure
     * A method to execute on each transaction data.
     ...
     * @param ?\Closure(mixed $data):void $transaction_done_closure
     * A closure to execute when a transaction is done
     *
     * @psalm-suppress MixedAssignment
     */
    public function __construct(
        array $process_transaction_data_iterator,
        \Closure $startup_closure,
        \Closure $transaction_closure,
        \Closure $shutdown_closure,
        ?\Closure $transaction_done_closure = null
    ) {
        $pool_size = count($process_transaction_data_iterator);
        $this->transaction_done_closure = $transaction_done_closure;
        ...
        $transaction_data_iterator = array_values($process_transaction_data_iterator)[$proc_id];

        $transaction_done_buffer = '';

        try {
            foreach ($transaction_data_iterator as $i => $transaction_data) {
                $transaction_result = $transaction_closure($i, $transaction_data);

                $transaction_done_message = new ForkTransactionDoneMessage($transaction_result);
                $serialized_message = $transaction_done_buffer . base64_encode(serialize($transaction_done_message)) . "\n";
                ...
                if (strlen($serialized_message) > 200) {
                    ...
                    if (strlen($serialized_message) !== $bytes_written) {
                        $transaction_done_buffer = substr($serialized_message, $bytes_written);
                    } else {
                        $transaction_done_buffer = '';
                    }
                } else {
                    $transaction_done_buffer = $serialized_message;
                }
            }
        ...
        $serialized_message = $transaction_done_buffer . base64_encode(serialize($process_done_message)) . "\n";
            ...
            foreach ($needs_read as $file) {
                ...
                if (strpos($buffer, "\n") !== false) {
                    ...
                    foreach ($serialized_messages as $serialized_message) {
                        ...
                        } elseif ($message instanceof ForkTransactionDoneMessage) {
                            if ($this->transaction_done_closure !== null) {
                                ($this->transaction_done_closure)($message->data);
                            }
